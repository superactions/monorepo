...
export type MockedObject<T> = T & {
  [P in keyof T]: T[P] extends (...args: any[]) => any ? MockFunction.Of<T[P]> : T[P]
}
...
export function mock<T>(overrides: Partial<T> = {}): MockedObject<T> {
  ...
  const proxy = new Proxy(clone as unknown as MockedObject<T>, {
    get(target, property, receiver) {
      ...
      const res = (): void => {
        throw new Error(`Cannot call .${name}() - no mock implementation provided.`)
      }
      ...
      Object.defineProperty(res, 'calls', {
        get: () => {
          throw new Error(
            'Cannot access .calls - no mock implementation provided' + ' and the function was not called.',
          )
        },
      })
...
function isMockFunction(x: unknown): x is Function {
